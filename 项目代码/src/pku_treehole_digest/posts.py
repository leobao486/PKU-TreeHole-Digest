...
def is_closed_badminton_offer(post: Post) -> bool:
    """Exclude badminton offers whose court/slot is explicitly no longer available."""
    searchable = " ".join([post.text, *post.comments, post.ai_summary]).lower()
    is_badminton = post.category == "羽毛球场地转让" or any(
        marker in searchable for marker in ("羽毛球", "邱德拔", "qdb", "五四")
    )
    if not is_badminton:
        return False
    closed_patterns = (
