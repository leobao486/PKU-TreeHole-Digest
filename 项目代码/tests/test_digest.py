...
from pku_treehole_digest.cli import (
    _adaptive_select,
    _is_current_interval_post,
)
...
from pku_treehole_digest.favorites import (
    add_favorite,
    create_folder,
    favorites_snapshot,
    mark_favorite_read,
...
from pku_treehole_digest.state import latest_report_path, load_latest_report_time
...
def test_closed_badminton_offer_is_excluded_but_open_offer_is_kept():
    closed = normalize_post({"pid": 1, "text": "出今晚邱德拔羽毛球场，已出。"})
    open_offer = normalize_post({"pid": 2, "text": "出今晚邱德拔羽毛球场，还有空位"})
    unrelated = normalize_post({"pid": 3, "text": "课程名额已满"})
    assert closed is not None and open_offer is not None and unrelated is not None
    closed.category = open_offer.category = "羽毛球场地转让"
    unrelated.category = "学习与课程"
    assert is_closed_badminton_offer(closed) is True
    assert is_closed_badminton_offer(open_offer) is False
    assert is_closed_badminton_offer(unrelated) is False
...
def test_hot_section_only_accepts_posts_created_in_current_report_window():
    now = datetime.now().astimezone()
    cutoff = now - timedelta(hours=6)
    current = normalize_post(
        {"pid": 20, "text": "current", "timestamp": int((now - timedelta(hours=1)).timestamp())}
    )
    old_post = normalize_post(
        {"pid": 21, "text": "old", "timestamp": int((cutoff - timedelta(hours=1)).timestamp())}
    )
    unknown_time = normalize_post({"pid": 22, "text": "unknown"})
    assert current is not None and old_post is not None and unknown_time is not None

    assert _is_current_interval_post(current, cutoff, now) is True
    assert _is_current_interval_post(old_post, cutoff, now) is False
    assert _is_current_interval_post(unknown_time, cutoff, now) is False


def test_priority_categories_precede_interests_and_post_heading_is_plain_number():
...
def test_rendered_html_has_sidebar_copy_button_collapsed_source_and_counts():
    ...
    assert post is not None
    post.category = "物理学习与课程"
    post.ai_summary = "这是一条值得关注的物理课程信息。"
    config = AppConfig(
        fetch=FetchConfig(),
    ...
    assert 'class="favorite-button"' in rendered
    assert 'id="favorites-drawer"' in rendered
    assert 'id="favorites-backdrop"' in rendered
    assert "expandedFavoriteFolders.clear()" in rendered
    assert ".favorite-folder.expanded .favorite-list" in rendered
    assert "window.setInterval(syncFavorites,3000)" in rendered
    assert "toggleFavorite('123456')" in rendered
    assert "const REPORT_POSTS=" in rendered
    assert 'id="favorite-post-modal"' in rendered
...
def test_reports_only_write_timestamped_html(tmp_path):
    first = datetime.now().astimezone()
    older = write_report("# useful", tmp_path, first)
    newer = write_report("# empty", tmp_path, first + timedelta(minutes=1))
    assert not (tmp_path / "latest.html").exists()
    timestamped = sorted(tmp_path.glob("*.html"))
...
def test_favorites_support_folders_moves_renames_and_unread_replies(monkeypatch, tmp_path):
    monkeypatch.setenv("LOCALAPPDATA", str(tmp_path))
    created = create_folder("课程")
    ...
    state = add_favorite(
        ...
        folder_id,
        {
            ...
            "summary": "summary",
            "replies": 2,
            "comments": [{"author": "Alice", "text": "a"}],
        },
    )
    assert state["items"][0]["unread"] is False
    default_id = state["folders"][0]["id"]
    moved = move_favorite("123", default_id)
    assert moved["items"][0]["folder_id"] == default_id
    renamed = rename_folder(default_id, "稍后阅读")
    assert renamed["folders"][0]["name"] == "稍后阅读"

    updated = add_favorite(
        "123",
        default_id,
        {"pid": "123", "text": "original", "replies": 5, "comments": []},
    )
    assert updated["items"][0]["unread"] is True
    assert updated["items"][0]["new_replies"] == 3
    read = mark_favorite_read("123")
    assert read["items"][0]["unread"] is False
    assert favorites_snapshot()["items"][0]["post"]["pid"] == "123"


def test_favorites_refresh_each_saved_post_by_pid_without_deepseek(monkeypatch, tmp_path):
    monkeypatch.setenv("LOCALAPPDATA", str(tmp_path))
    add_favorite("321", "default", {"pid": "321", "text": "old", "replies": 1})
    ...
    class FakeClient:
        ...
        def enrich_post(self, post, comment_page_size):
            return post

    client = FakeClient()
    refreshed = refresh_favorites(client, 50)
    assert client.pids == ["321"]
    assert refreshed["items"][0]["post"]["text"] == "refreshed"
    assert refreshed["items"][0]["unread"] is True
    assert refreshed["items"][0]["new_replies"] == 2
